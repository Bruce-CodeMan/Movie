...
//获取正在热播
func GetHotList(isHot int) []Video {
	var info []Video
	req := httplib.Get(beego.AppConfig.String("apiurl") + "/channel/hot?isHot=" + strconv.Itoa(isHot))
...
// 获取电视剧推荐
func GetEpisodesRecommend(channelId int) []Video {
	var info []Video
	req := httplib.Get(beego.AppConfig.String("apiurl") + "/episode/recommend?channelId=" + strconv.Itoa(channelId) + "")
	str, err := req.String()
	if err != nil {
		fmt.Println(err)
	}
...
// 获取电影推荐
func GetMoviesRecommend(channelId int) []Video {
	var info []Video
	req := httplib.Get(beego.AppConfig.String("apiurl") + "/movie/recommend?channelId=" + strconv.Itoa(channelId) + "")
	str, err := req.String()
	if err != nil {
		fmt.Println(err)
	}
